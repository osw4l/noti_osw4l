defmodule NotiOsw4lWeb.ActivityLogLive do
  ...
  def render(assigns) do
    ...
    <div class="max-w-4xl mx-auto mt-8">
      ...
      <div :if={@logs == []} class="text-center py-12 text-zinc-400">
        No hay actividad registrada aún.
      </div>

      <div class="space-y-2">
        <div :for={log <- @logs} class="flex items-start gap-3 p-3 border-b">
          ...
          <div class="flex-1">
            <p class="text-sm">
              ...
              <span class="text-zinc-600">{action_label(log.action)}</span>
              <span class="font-medium">{log.entity_type}</span>
              <span :if={log.metadata["title"]} class="text-zinc-500">
                "{log.metadata["title"]}"
              </span>
            </p>
  ...
  defp action_color("created"), do: "bg-green-500"
  defp action_color("deleted"), do: "bg-red-500"
  defp action_color("completed"), do: "bg-blue-500"
  defp action_color("uncompleted"), do: "bg-yellow-500"
  defp action_color(_), do: "bg-zinc-400"
